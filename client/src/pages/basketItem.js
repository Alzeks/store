...
deleteDevice(id).then(data => {
 setDestroyDevice(true); changeTotalCount();
...
countDevice(id, count).then(data =>
  ...
  return (
  //  <Button  variant={'outline-dark text-light'}>delete</Button>
  //  <Image style={{wigth: 200, cursor: 'pointer'}} src={'http://localhost:5000/' + el.img}/>
<div>{destroyDevice ||
